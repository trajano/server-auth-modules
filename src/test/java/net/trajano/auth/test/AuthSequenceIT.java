...
public class AuthSequenceIT {
	...
	public void testRedirectToEndpoint() throws Exception {
		...
		final Map<String, String> options = ImmutableMap.<String, String>builder()
				...
				.put(REDIRECTION_ENDPOINT_URI_KEY, "/app/oauth2").put(OAuthModule.CLIENT_ID_KEY, "clientID")
		...
		verify(servletResponse).sendRedirect(redirectCaptor.capture());
		assertTrue(redirectCaptor.getValue(), redirectCaptor.getValue().startsWith(
				"https://accounts.google.com/o/oauth2/v2/auth?client_id=clientID&response_type=code&scope=openid&redirect_uri=https://i.trajano.net:8443/app/oauth2&state=L2VqYjI"));
	}
	...
	public void testRedirectToEndpointWithQueryString() throws Exception {
		...
		ArgumentCaptor<String> redirectCaptor = ArgumentCaptor.forClass(String.class);
		verify(servletResponse).sendRedirect(redirectCaptor.capture());
		assertTrue(redirectCaptor.getValue().startsWith(
				"https://accounts.google.com/o/oauth2/v2/auth?client_id=clientID&response_type=code&scope=openid&redirect_uri=https://i.trajano.net:8443/app/oauth2&state=L2VqYjI"));
	}
}
